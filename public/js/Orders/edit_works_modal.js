...
function initAutonumeric() {
    //set colones radio always the page is reloaded
...
// This function disables the radio buttons of money
...
function fixProductDateFormat() {
    var table = $('#worksTable').DataTable();
    rowCount = table.rows().count();
    for (var i = 0; i < rowCount; i++) {

        date = $("#date" + i).html();
        $("#date" + i).html(trimDeleteEnters(date));

        product = $("#product" + i).html();
        $("#product" + i).html(trimDeleteEnters(product));
    }
}

function trimDeleteEnters(text) {
    text = text.replace(/\r?\n|\r/g, '');
    text = text.trim();
    return text;
}
